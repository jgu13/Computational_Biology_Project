...
cmask = opening(CD3_cell_surface.norm.blur>0.1, makeBrush(3, shape = 'disc'))
display(cmask)
cmask = propagate(CD3_cell_surface.norm.blur, seeds = nmask, mask = cmask, lambda = 50)

display(CD3_cell_surface.norm.blur>0.1)
display(colorLabels(cmask))
display(paintObjects(cmask, CD3_cell_surface.norm.blur, col = "#ff00ff"))
...
CD_8a_cell_surface = data$`21RD`$CD8a
#hist(CD_8a_cell_surface)
#display(CD_8a_cell_surface)
CD_8a_cell_surface.norm = normalize(CD_8a_cell_surface, inputRange = quantile(CD_8a_cell_surface, c(0,0.99)))
#hist(CD_8a_cell_surface.norm)
CD_8a_cell_surface.norm.blur = gblur(CD_8a_cell_surface.norm, sigma = 1)
display(CD_8a_cell_surface.norm.blur)

cells = rgbImage(green = 1.5*CD_8a_cell_surface.norm.blur, blue = 1.5*img_histone.norm.blur)

#cell surface(regions) mask
#CD3_cell_surface.norm.blur = gblur(CD3_cell_surface, sigma = 1)
cmask = opening(CD_8a_cell_surface.norm.blur>0.1, makeBrush(5, shape = 'disc'))
display(cmask)
cmask = propagate(CD_8a_cell_surface.norm.blur, seeds = nmask, mask = cmask)

display(CD_8a_cell_surface.norm.blur>0.1)
display(colorLabels(cmask))
display(paintObjects(cmask, CD_8a_cell_surface.norm.blur, col = "#ff00ff"))
#paint outline of cell mask on image of cells
segmented = paintObjects(cmask, cells, col = "#ff00ff")
